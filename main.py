...
def select_action(dat):
    ...
    if state == 'self_play':
        ...
        if angang_res != False and is_last(dat, True, isGang) == False:
            global_max_score, global_action = angang_res
        if bugang_res != False and is_last(dat, True, isGang) == False:
        ...
        if gang_res != False and is_last(dat, False, False) == False:
        ...
        if peng_res != False and is_last(dat, False, False) == False:
        ...
        if chi_res != False and is_last(dat, False, False) == False:
...
def cal_score(dat, tables):
    ...
    weight = calWeight(dat)

    # 枚举将在每一种花色的情况
    for jiang_type in range(5):
        is_jiang = [0] * 5
        is_jiang[jiang_type] = 1
        cur_score = sum([cur_table[cur_key][cur_is_jiang] for cur_table, cur_key, cur_is_jiang in
                         zip(all_tables, all_keys, is_jiang)])
        if cur_score > max_score:
            jiang_selected = jiang_type
            max_score = cur_score

    return max_score * weight, jiang_selected


def calWeight(dat):
    all_card_kind = set()
    for card in dat["avail_cards"]:
        if card[0] in ["T", "B", "W"]:
            if card[1] != "1":
                all_card_kind.add("%s%d" % (card[0],int(card[1])-1))
            all_card_kind.add("%s%d" % (card[0], int(card[1])))
            if card[1] != "9":
                all_card_kind.add("%s%d" % (card[0], int(card[1])+1))
        elif card[0] in ["F", "J"]:
            all_card_kind.add(card)
    for card in list(all_card_kind):
        if dat["all_shown_cards"].count(card) == 4:
            all_card_kind.remove(card)
    Weight = 1
    ## 一轮搜索，仅限于已经听胡的牌型
    for card in all_card_kind:
        cur_fan = naive_cal_fan(dat, card)
        if cur_fan >= 8:
            remaining_card_cnt = 4 - dat["all_shown_cards"].count(card)
            Weight = Weight + cur_fan * remaining_card_cnt

    ## 二轮搜索，还差一张牌听胡的牌型：
    for card_add in all_card_kind:
        for card_sub in set(dat["avail_cards"]):
            ## 一样的牌代表一轮搜索已经搜过
            if card_add == card_sub:
                continue
            remaining_card_add_cnt = 4 - dat["all_shown_cards"].count(card_add)
            dat["avail_cards"].append(card_add)
            dat["avail_cards"].remove(card_sub)
            dat["all_shown_cards"].append(card_add)
            dat["all_shown_cards"].append(card_sub)
            for card in all_card_kind:
                cur_fan = naive_cal_fan(dat, card)
                if cur_fan >= 8:
                    remaining_card_cnt = 4 - dat["all_shown_cards"].count(card)
                    Weight = Weight + cur_fan * remaining_card_cnt * remaining_card_add_cnt * 0.01
            dat["avail_cards"].remove(card_add)
            dat["avail_cards"].append(card_sub)
            dat["all_shown_cards"].remove(card_add)
            dat["all_shown_cards"].remove(card_sub)
    return Weight
...
def play_card(dat, tables, policy='table'):
    ...
    if policy == 'random':
        ...
        for card in set(dat['avail_cards']):
            ...
            cur_score, jiang_selected = cal_score(dat, tables)
...
def chi_card(dat, tables, policy='table'):
    ...
    if policy == 'table':
        ...
        raw_score, _ = cal_score(dat, tables)
        max_score = raw_score
        ...
        if dat['avail_cards'].count(card_l2) > 0 and dat['avail_cards'].count(card_l1) > 0:
            ...
            new_score, _ = cal_score(dat, tables)
...
def naive_cal_fan(dat, card):
    ...
    if dat['all_shown_cards'].count(card) == 3:
        isJuezhang = True
    try:
        judge_res = MahjongFanCalculator(cur_pack, cur_hand, card, 0, isZimo, isJuezhang, isGang, isLast, dat['quan'],
                                         dat['id'])
...
def judge_hu(dat, isZimo, isGang):
    ...
    try:
        judge_res = MahjongFanCalculator(cur_pack, cur_hand, winTile, 0, isZimo, isJuezhang, isGang, isLast,
                                         dat['quan'], dat['id'])
    except Exception as err:
        judge_res = str(err)
...
def unit_test():
    temp_dat = {
        ...
        "quan": 3,
        "all_shown_cards": ['F2', 'F3', 'J1', 'J3', 'J3', 'B1', 'T9', 'F2', 'J2', 'W5', 'W6', 'W7', 'B8', 'B2', 'T9',
                            'T3', 'T9', 'B9', 'F4', 'F4', 'F4', 'T2', 'T3', 'T4', 'W5', 'W8', 'B8', 'J2', 'W1', 'B3',
                            'B4', 'B5', 'W9', 'T5', 'T2', 'B2', 'T5', 'T1', 'W9', 'F2', 'B8', 'W7', 'B6', 'B9', 'J3',
                            'T9', 'B7', 'W1', 'J1', 'W2', 'W2', 'W3', 'W4', 'T4', 'F2', 'F3', 'F3', 'F3', 'T7', 'T7',
                            'T7', 'B9', 'T3', 'T7', 'B4', 'F4', 'B2', 'T5', 'W6', 'B8', 'J2', 'B2', 'W8', 'J2', 'B1',
                            'B6', 'B4', 'T6', 'T6', 'T6', 'W2', 'J3', 'J1', 'B7', 'B3', 'W6', 'T8', 'B5', 'B3', 'B4',
                            'T2', 'T2', 'W4', 'B5', 'T8', 'B1', 'W1', 'W2', 'B6', 'B9', 'W7', 'T6', 'F1'],
        "last_is_gang": False,
...
if __name__ == "__main__":
    ...
    """
    dat = {
        "cards": ["T1","T1","T1","T2","T3","T4","T5","T6","T7","T8","T9","T9","F1"],
        "avail_cards": ["T1","T1","T1","T2","T3","T4","T5","T6","T7","T8","T9","T9","F1"],
        "all_shown_cards": [],
        "pack": [],
        "quan": 3,
        "id": 3
